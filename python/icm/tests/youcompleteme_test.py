...
from icm.tests.test_utils import ( ExtendedMock, MockVimBuffers, MockVimModule,
                                   VimBuffer, VimMatch )
MockVimModule()
...
def YouCompleteMe_YcmCoreNotImported_test( ycm ):
  assert_that( 'icm_core', is_not( is_in( sys.modules ) ) )


@YouCompleteMeInstance()
@patch( 'icm.vimsupport.PostVimMessage', new_callable = ExtendedMock )
...
def YouCompleteMe_NotifyUserIfServerCrashed_UnexpectedCore_test():
  message = ( "The ycmd server SHUT DOWN (restart with ':IcmRestartServer'). "
              "Unexpected error while loading the YCM core library. "
              "Use the ':IcmToggleLogs' command to check the logs." )
...
def YouCompleteMe_NotifyUserIfServerCrashed_Python2Core_test():
  message = ( "The ycmd server SHUT DOWN (restart with ':IcmRestartServer'). "
              ...
              "Set the 'g:icm_server_python_interpreter' option to a Python 2 "
              "interpreter path." )
  RunNotifyUserIfServerCrashed( {
    'return_code': 5,
    'stderr_output': '',
...
def YouCompleteMe_NotifyUserIfServerCrashed_Python3Core_test():
  message = ( "The ycmd server SHUT DOWN (restart with ':IcmRestartServer'). "
              ...
              "Set the 'g:icm_server_python_interpreter' option to a Python 3 "
              "interpreter path." )
  RunNotifyUserIfServerCrashed( {
    'return_code': 6,
    'stderr_output': '',
...
def YouCompleteMe_NotifyUserIfServerCrashed_UnexpectedExitCode_test():
  message = ( "The ycmd server SHUT DOWN (restart with ':IcmRestartServer'). "
              "Unexpected exit code 1. Use the ':IcmToggleLogs' command to "
              "check the logs." )
  RunNotifyUserIfServerCrashed( {
    'return_code': 1,
...
@YouCompleteMeInstance()
@patch( 'icm.vimsupport.CloseBuffersForFilename', new_callable = ExtendedMock )
@patch( 'icm.vimsupport.OpenFilename', new_callable = ExtendedMock )
...
@YouCompleteMeInstance()
@patch( 'icm.client.base_request._logger', autospec = True )
@patch( 'icm.vimsupport.PostVimMessage', new_callable = ExtendedMock )
...
@YouCompleteMeInstance()
@patch( 'icm.youcompleteme.YouCompleteMe.FiletypeCompleterExistsForFiletype',
        return_value = True )
@patch( 'icm.vimsupport.PostVimMessage', new_callable = ExtendedMock )
@patch( 'icm.vimsupport.SetLocationList', new_callable = ExtendedMock )
def YouCompleteMe_ShowDiagnostics_NoDiagnosticsDetected_test(
  ...
  with MockVimBuffers( [ current_buffer ], current_buffer ):
    with patch( 'icm.client.event_notification.EventNotification.Response',
                return_value = {} ):
      ycm.ShowDiagnostics()
...
@YouCompleteMeInstance( { 'open_loclist_on_icm_diags': 1 } )
@patch( 'icm.youcompleteme.YouCompleteMe.FiletypeCompleterExistsForFiletype',
        return_value = True )
@patch( 'icm.vimsupport.PostVimMessage', new_callable = ExtendedMock )
@patch( 'icm.vimsupport.SetLocationList', new_callable = ExtendedMock )
@patch( 'icm.vimsupport.OpenLocationList', new_callable = ExtendedMock )
...
def YouCompleteMe_UpdateDiagnosticInterface_PrioritizeErrorsOverWarnings_test(
  ...
  with MockVimBuffers( [ current_buffer ], current_buffer, ( 3, 1 ) ):
    ...
    assert_that(
      test_utils.VIM_MATCHES,
      contains(
        VimMatch( 'IcmWarningSection', '\%3l\%5c\_.\{-}\%3l\%7c' ),
        VimMatch( 'IcmWarningSection', '\%3l\%3c\_.\{-}\%3l\%9c' ),
        VimMatch( 'IcmErrorSection', '\%3l\%8c' )
      )
    )

    # Only the error sign is placed.
    vim_command.assert_has_exact_calls( [
      call( 'sign define icm_dummy_sign' ),
      call( 'sign place 3 name=icm_dummy_sign line=3 buffer=5' ),
      call( 'sign place 1 name=IcmError line=3 buffer=5' ),
      call( 'sign undefine icm_dummy_sign' ),
