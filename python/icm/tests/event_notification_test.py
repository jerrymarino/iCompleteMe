...
from icm.tests import PathToTestFile, YouCompleteMeInstance, WaitUntilReady
...
def MockEventNotification( response_method, native_filetype_completer = True ):
  ...
  # return success
  with patch( 'icm.client.event_notification.EventNotification.'
    ...
    # for details.
    with patch( 'icm.client.event_notification.JsonFromFuture',
      ...
      with patch(
        'icm.youcompleteme.YouCompleteMe.FiletypeCompleterExistsForFiletype',
...
@patch( 'icm.client.base_request._LoadExtraConfFile',
        new_callable = ExtendedMock )
@patch( 'icm.client.base_request._IgnoreExtraConfFile',
        new_callable = ExtendedMock )
@YouCompleteMeInstance()
def EventNotification_FileReadyToParse_NonDiagnostic_ConfirmExtraConf_test(
  ...
  with MockArbitraryBuffer( 'javascript' ):
    with MockEventNotification( UnknownExtraConfResponse ):
      ...
      with patch( 'icm.vimsupport.PresentDialog',
                  return_value = 0,
...
@patch( 'icm.youcompleteme.YouCompleteMe._AddUltiSnipsDataIfNeeded' )
...
@patch( 'icm.syntax_parse.SyntaxKeywordsForCurrentBuffer',
