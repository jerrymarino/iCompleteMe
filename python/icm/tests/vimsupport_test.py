...
from icm.tests import PathToTestFile
from icm.tests.test_utils import ( CurrentWorkingDirectory, ExtendedMock,
...
@patch( 'icm.vimsupport.VariableExists', return_value = True )
@patch( 'icm.vimsupport.SetFittingHeightForCurrentWindow' )
...
def OpenLocationList_test( vim_command, fitting_height, variable_exists ):
  ...
  vim_command.assert_has_exact_calls( [
    ...
    call( 'doautocmd User IcmLocationOpened' ),
    call( 'silent! wincmd p' )
  ] )
  fitting_height.assert_called_once_with()
  variable_exists.assert_called_once_with( '#User#IcmLocationOpened' )


@patch( 'icm.vimsupport.GetIntValue', return_value = 120 )
@patch( 'vim.command' )
...
@patch( 'icm.vimsupport.VariableExists', return_value = False )
@patch( 'icm.vimsupport.SetFittingHeightForCurrentWindow' )
@patch( 'icm.vimsupport.GetBufferNumberForFilename',
        return_value = 1,
        new_callable = ExtendedMock )
@patch( 'icm.vimsupport.BufferIsVisible',
        return_value = True,
        new_callable = ExtendedMock )
@patch( 'icm.vimsupport.OpenFilename' )
...
@patch( 'icm.vimsupport.VariableExists', return_value = False )
@patch( 'icm.vimsupport.SetFittingHeightForCurrentWindow' )
@patch( 'icm.vimsupport.GetBufferNumberForFilename', side_effect = [
          22, # first_file (check)
          -1, # second_file (check)
          22, # first_file (apply)
          -1, # second_file (apply)
        ...
        new_callable = ExtendedMock )
@patch( 'icm.vimsupport.BufferIsVisible', side_effect = [
...
@patch( 'icm.vimsupport.PostVimMessage',
        new_callable = ExtendedMock)
@patch( 'icm.vimsupport.Confirm', return_value = True,
        new_callable = ExtendedMock)
...
def AddDiagnosticSyntaxMatch_ErrorInMiddleOfLine_test( vim_eval ):
  ...
  vim_eval.assert_called_once_with(
    r"matchadd('IcmErrorSection', '\%1l\%16c\_.\{-}\%1l\%21c')" )
...
def AddDiagnosticSyntaxMatch_WarningAtEndOfLine_test( vim_eval ):
  ...
  vim_eval.assert_called_once_with(
    r"matchadd('IcmWarningSection', '\%1l\%16c\_.\{-}\%1l\%23c')" )
...
@patch( 'icm.vimsupport.CurrentColumn', side_effect = [ 4 ] )
...
@patch( 'icm.vimsupport.CurrentColumn', side_effect = [ 2 ] )
...
@patch( 'icm.vimsupport.VariableExists', return_value = False )
@patch( 'icm.vimsupport.SearchInCurrentBuffer', return_value = 0 )
@patch( 'vim.current' )
...
@patch( 'icm.vimsupport.VariableExists', return_value = False )
@patch( 'icm.vimsupport.SearchInCurrentBuffer', return_value = 2 )
@patch( 'vim.current' )
