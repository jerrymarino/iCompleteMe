...
from icm import base


@contextlib.contextmanager
def MockCurrentFiletypes( filetypes = [''] ):
  with patch( 'icm.vimsupport.CurrentFiletypes', return_value = filetypes ):
    yield


@contextlib.contextmanager
def MockCurrentColumnAndLineContents( column, line_contents ):
  with patch( 'icm.vimsupport.CurrentColumn', return_value = column ):
    with patch( 'icm.vimsupport.CurrentLineContents',
...
def MockTextAfterCursor( text ):
  with patch( 'icm.vimsupport.TextAfterCursor', return_value = text ):
    yield
