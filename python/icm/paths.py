...
# `g:icm_server_python_interpreter` option.
def PathToPythonInterpreter():
  # Not calling the Python interpreter to check its version as it significantly
  # impacts startup time.
  from ycmd import utils

  python_interpreter = vim.eval( 'g:icm_server_python_interpreter' )
  if python_interpreter:
    if _EndsWithPython( python_interpreter ):
      return python_interpreter

    raise RuntimeError( "Path in 'g:icm_server_python_interpreter' option "
  ...
  raise RuntimeError( "Cannot find Python 2.6+ or 3.3+. You can set its path "
                      "using the 'g:icm_server_python_interpreter' "
                      "option." )
