...
def OpenLocationList( focus = False, autoclose = False ):
  ...
  by using the IcmLocationOpened autocommand. When focus is set to True, the
  ...
  if VariableExists( '#User#IcmLocationOpened' ):
    vim.command( 'doautocmd User IcmLocationOpened' )
...
def OpenQuickFixList( focus = False, autoclose = False ):
  ...
  height automatically set to fit all entries. This behavior can be overridden
  by using the IcmQuickFixOpened autocommand.
  ...
  if VariableExists( '#User#IcmQuickFixOpened' ):
    vim.command( 'doautocmd User IcmQuickFixOpened' )
...
def InsertNamespace( namespace ):
  if VariableExists( 'g:icm_csharp_insert_namespace_expr' ):
    expr = GetVariableValue( 'g:icm_csharp_insert_namespace_expr' )
    if expr:
      SetVariableValue( "g:icm_namespace_to_insert", namespace )
      vim.eval( expr )
      return
