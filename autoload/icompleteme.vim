...
function! icompleteme#Enable()
  ...
  augroup icompleteme
    ...
    " is read. This is because icompleteme#Enable() is called on VimEnter and
...
function! icompleteme#EnableCursorMovedAutocommands()
  augroup icmcompletemecursormove
    autocmd!
...
function! icompleteme#DisableCursorMovedAutocommands()
  autocmd! icmcompletemecursormove
endfunction


function! icompleteme#GetErrorCount()
  return s:Pyeval( 'icm_state.GetErrorCount()' )
endfunction


function! icompleteme#GetWarningCount()
  return s:Pyeval( 'icm_state.GetWarningCount()' )
endfunction
...
from icm.setup import SetUpSystemPaths as ICMSetUpSystemPaths
from icm.setup import SetUpYCM
...
try:
  ...
  from icm import base as icm_base
  from icm import vimsupport as icm_vimsupport
...
function! s:SetUpKeyMappings()
  " The g:icm_key_select_completion and g:icm_key_previous_completion used to
  " exist and are now here purely for the sake of backwards compatibility; we
  " don't want to break users if we can avoid it.

  if exists('g:icm_key_select_completion') &&
        \ index(g:icm_key_list_select_completion,
        \       g:icm_key_select_completion) == -1
    call add(g:icm_key_list_select_completion, g:icm_key_select_completion)
  endif

  if exists('g:icm_key_previous_completion') &&
        \ index(g:icm_key_list_previous_completion,
        \       g:icm_key_previous_completion) == -1
    call add(g:icm_key_list_previous_completion, g:icm_key_previous_completion)
  endif

  for key in g:icm_key_list_select_completion
  ...
  for key in g:icm_key_list_previous_completion
    " This selects the previous candidate for shift-tab (default)
    exe 'inoremap <expr>' . key .
          \ ' pumvisible() ? "\<C-p>" : "\' . key .'"'
  endfor

  if !empty( g:icm_key_invoke_completion )
    let invoke_key = g:icm_key_invoke_completion
  ...
  if !empty( g:icm_key_detailed_diagnostics )
    silent! exe 'nnoremap <unique> ' . g:icm_key_detailed_diagnostics .
          \ ' :IcmShowDetailedDiagnostic<cr>'
  endif
endfunction
...
function! s:DisableOnLargeFile( buffer )
  if exists( 'b:icm_largefile' )
    return b:icm_largefile
  endif

  let threshold = g:icm_disable_for_files_larger_than_kb * 1024
  let b:icm_largefile =
        \ threshold > 0 && getfsize( expand( a:buffer ) ) > threshold
  if b:icm_largefile
    exec s:python_command "icm_vimsupport.PostVimMessage(" .
          \ "'iCompleteMe is disabled in this buffer; " .
...
function! s:AllowedToCompleteInBuffer( buffer )
  ...
  let whitelist_allows = has_key( g:icm_filetype_whitelist, '*' ) ||
        \ has_key( g:icm_filetype_whitelist, buffer_filetype )
  let blacklist_allows = !has_key( g:icm_filetype_blacklist, buffer_filetype )
...
function! s:SetUpCompleteopt()
  ...
  if g:icm_add_preview_to_completeopt
...
function! s:OnVimLeave()
  exec s:python_command "icm_state.OnVimLeave()"
endfunction
...
function! s:OnFileTypeSet()
  ...
  exec s:python_command "icm_state.OnBufferVisit()"
...
function! s:OnBufferUnload()
  ...
  exec s:python_command "icm_state.OnBufferUnload(" .
        \ "vim.eval( 'deleted_buffer_file' ) )"
endfunction


function! s:PollServerReady( timer_id )
  if !s:Pyeval( 'icm_state.IsServerReady()' )
    let s:pollers.server_ready.id = timer_start(
          \ s:pollers.server_ready.wait_milliseconds,
          \ function( 's:PollServerReady' ) )
    return
...
function! s:OnFileReadyToParse( ... )
  ...
  if force_parsing || b:changedtick != get( b:, 'icm_changedtick', -1 )
    exec s:python_command "icm_state.OnFileReadyToParse()"
    ...
    let b:icm_changedtick = b:changedtick
  endif
endfunction


function! s:PollFileParseResponse( ... )
  if !s:Pyeval( "icm_state.FileParseRequestReady()" )
    let s:pollers.file_parse_response.id = timer_start(
          \ s:pollers.file_parse_response.wait_milliseconds,
          \ function( 's:PollFileParseResponse' ) )
    return
  ...
  exec s:python_command "icm_state.HandleFileParseRequest()"
endfunction


function! s:SetCompleteFunc()
  let &completefunc = 'icompleteme#Complete'
  let &l:completefunc = 'icompleteme#Complete'
endfunction


function! s:SetOmnicompleteFunc()
  if s:Pyeval( 'icm_state.NativeFiletypeCompletionUsable()' )
    let &omnifunc = 'icompleteme#OmniComplete'
    let &l:omnifunc = 'icompleteme#OmniComplete'
  ...
  elseif &omnifunc == 'icompleteme#OmniComplete'
    let &omnifunc = ''
    let &l:omnifunc = ''
  endif
endfunction
...
function! s:OnTextChangedInsertMode()
  ...
  if g:icm_autoclose_preview_window_after_completion
    call s:ClosePreviewWindowIfNeeded()
  endif

  if g:icm_auto_trigger || s:omnifunc_mode
  ...
  if s:omnifunc_mode && !s:Pyeval( 'icm_base.LastEnteredCharIsIdentifierChar()')
    let s:omnifunc_mode = 0
  endif
endfunction
...
function! s:OnInsertLeave()
  ...
  let s:omnifunc_mode = 0
  call s:OnFileReadyToParse()
  exec s:python_command "icm_state.OnInsertLeave()"
  if g:icm_autoclose_preview_window_after_completion ||
        \ g:icm_autoclose_preview_window_after_insertion
    call s:ClosePreviewWindowIfNeeded()
  endif
endfunction
...
function! s:IdentifierFinishedOperations()
  if !s:Pyeval( 'icm_base.CurrentIdentifierFinished()' )
    return
  endif
  exec s:python_command "icm_state.OnCurrentIdentifierFinished()"
  let s:omnifunc_mode = 0
endfunction
...
function! s:InsideCommentOrStringAndShouldStop()
  ...
  if inside_comment && g:icm_complete_in_comments ||
        \ inside_string && g:icm_complete_in_strings
    return 0
  endif
...
function! s:InvokeCompletion()
  if &completefunc != "icompleteme#Complete"
    return
  endif
  ...
  " icompleteme#Complete), and <c-p> tells Vim to select the previous
...
function! icompleteme#Complete( findstart, base )
  ...
  if s:omnifunc_mode
    return icompleteme#OmniComplete( a:findstart, a:base )
  endif
  ...
  if a:findstart
    ...
    exec s:python_command "icm_state.CreateCompletionRequest()"
    return s:Pyeval( 'icm_base.CompletionStartColumn()' )
  else
    return s:Pyeval( 'icm_state.GetCompletions()' )
  endif
endfunction


function! icompleteme#OmniComplete( findstart, base )
  if a:findstart
...
function! icompleteme#ServerPid()
  return s:Pyeval( 'icm_state.ServerPid()' )
endfunction


function! s:SetUpCommands()
  command! IcmRestartServer call s:RestartServer()
  command! IcmDebugInfo call s:DebugInfo()
  command! -nargs=* -complete=custom,icompleteme#LogsComplete
        \ IcmToggleLogs call s:ToggleLogs(<f-args>)
  command! -nargs=* -complete=custom,icompleteme#SubCommandsComplete
        \ IcmCompleter call s:CompleterCommand(<f-args>)
  command! IcmDiags call s:ShowDiagnostics()
  command! IcmShowDetailedDiagnostic call s:ShowDetailedDiagnostic()
  command! IcmForceCompileAndDiagnostics call s:ForceCompileAndDiagnostics()
endfunction


function! s:RestartServer()
  exec s:python_command "icm_state.RestartServer()"
...
function! s:DebugInfo()
  echom "Printing iCompleteMe debug information..."
  let debug_info = s:Pyeval( 'icm_state.DebugInfo()' )
...
function! s:ToggleLogs(...)
  exec s:python_command "icm_state.ToggleLogs( *vim.eval( 'a:000' ) )"
endfunction


function! icompleteme#LogsComplete( arglead, cmdline, cursorpos )
  return join( s:Pyeval( 'list( icm_state.GetLogfiles() )' ), "\n" )
endfunction
...
function! s:CompleterCommand(...)
  ...
  " "ft=icm:ident" to select the identifier completer.
  ...
  exec s:python_command "icm_state.SendCommandRequest(" .
        \ "vim.eval( 'l:arguments' ), vim.eval( 'l:completer' ) )"
endfunction


function! icompleteme#SubCommandsComplete( arglead, cmdline, cursorpos )
  return join( s:Pyeval( 'icm_state.GetDefinedSubcommands()' ), "\n" )
endfunction


function! icompleteme#OpenGoToList()
  exec s:python_command "icm_vimsupport.PostVimMessage(" .
        \ "'WARNING: icompleteme#OpenGoToList function is deprecated. " .
        \ "Do NOT use it.' )"
  exec s:python_command "icm_vimsupport.OpenQuickFixList( True, True )"
endfunction


function! s:ShowDiagnostics()
  exec s:python_command "icm_state.ShowDiagnostics()"
endfunction


function! s:ShowDetailedDiagnostic()
  exec s:python_command "icm_state.ShowDetailedDiagnostic()"
endfunction


function! s:ForceCompileAndDiagnostics()
  exec s:python_command "icm_state.ForceCompileAndDiagnostics()"
endfunction
